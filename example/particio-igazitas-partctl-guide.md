...
- **Alapértelmezett vég LBA:** ha a kiválasztott szabad sáv abszolút vége **`N`** szektor (parted `Ns`), a javasolt alapértelmezés **nem** `Ns`. Először levonunk egy **1 MiB**-nek megfelelő szektorszámot — **`mib_step`** = `ceil(1 MiB / logikai szektor)` (512 B-nél **2048** → elvben **„N − 2048s”**; 4096 B-nél **256** → **„N − 256s”**). Ezután a végszektort **32 szektoros** határra igazítjuk (`sgdisk` kompatibilitás), ezért 512 B-nél a képernyőn látható vég gyakran **`N − 2048` … `N − 2080`** környékén esik, nem feltétlenül pontosan `N−2048s`. A **„Vég”** mezőt **kézzel bármikor felülírhatod** (pl. tényleges `Ns` maximumra, ha tudatosan minden szektort fel akarsz használni).
...
**Mit jelent a gyakorlatban?** Ha a varázsló a kezdetnél **`2048s`**-t, a végnél pl. **`488395039s`**-t kínál fel egy ~233 GiB-os szabad sávon, akkor a partíció **fizikailag** kb. **`free_end − 2048..2080`** szektoron ér véget, így a lemez legutolsó **1–2 MiB-ja** szabad marad. Ez tudatos tervezés — **ne** írd át nullára a véget abszolút `free_end`-re, hacsak nem konkrét okod van rá (pl. nem-GPT, nem-LUKS, és minden bájt számít).
